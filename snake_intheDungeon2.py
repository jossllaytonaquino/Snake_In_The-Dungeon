...
def game_loop(level):
    ...
    num_obstacles = 5 + (level * 3)  # 5 for level 1, 8 for level 2, 11 for level 3
    obstacles = []
    while len(obstacles) < num_obstacles:
        new_obs = [random.randrange(1, (WIDTH // SNAKE_SIZE)) * SNAKE_SIZE,
                   random.randrange(1, (HEIGHT // SNAKE_SIZE)) * SNAKE_SIZE]
        if new_obs not in snake_body and new_obs != food_pos and new_obs not in obstacles:
